...
from django.contril.auth.decorators import login_required
    ...
    if request.method == 'POST':
        ...
        if request.POST['mode'] == '__friends_form__':
            ...
            for item in fds:
                ...
                vlist.append(item.user.username)
            # メッセージを設定
            messages.sucsess(sequest, ' チェックされたFriendを' + sel_group + 'に登録しました。')
            # フォームの用意
...
def add(request):
    ...
    frd.save()
    # メッセージ設定
    messages.success(request, add_user.username + ' を追加しました！グループページに移動して、追加したFriendをメンバーに設定してください。')
    return redirect(to='/sns')
...
def good(request, good_id):
    ...
    is_good = Good.objects.filter(pwner=request.user).filter(message=good_msg).count()
    # ゼロより大きければすでにgood済み
    if is_good > 0:
        messages.success(request, 'すでにメッセージにはgoodしてます。')
        return redirect(to='/sns')
